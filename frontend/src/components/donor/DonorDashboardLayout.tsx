...
export type DonorDashboardLayoutMode = "donor" | "admin";

export type DonorDashboardLayoutProps = {
  data: DonorDashboard;
  mode: DonorDashboardLayoutMode;
...
export function DonorDashboardLayout({
  ...
  const resolvedEyebrow =
    heroEyebrow ?? (mode === "admin" ? "Donor record" : "Supporter dashboard");
  const resolvedTitle = heroTitle ?? (mode === "admin" ? fullName : `Welcome, ${fullName}`);
  ...
  const profileRows =
    mode === "admin"
      ? ([
          ...
          { label: "Phone", value: data.supporter.phone ?? "N/A" },
          { label: "Region", value: data.supporter.region ?? "N/A" },
          { label: "Country", value: data.supporter.country ?? "N/A" },
          { label: "Status", value: data.supporter.status ?? "N/A" },
          { label: "Organization", value: data.supporter.organizationName ?? "N/A" },
          { label: "Relationship", value: data.supporter.relationshipType ?? "N/A" },
        ] as const)
      : ([
          { label: "Name", value: fullName },
          { label: "Email", value: data.supporter.email ?? "N/A" },
  ...
  const overviewHeading =
    mode === "admin" ? "Giving at a glance" : "Your giving at a glance";
  const overviewBody =
    mode === "admin"
      ? "Monetary and non-monetary contributions, program allocations, and recent activity for this donor."
      : "Track your monetary and non-monetary contributions, review your impact, and continue supporting Beacon's mission.";
  const profileEyebrow = mode === "admin" ? "Profile" : "Your profile";
          ...
          <div className="row g-4 align-items-stretch mb-4">
            <div className={mode === "donor" ? "col-lg-8" : "col-12"}>
              <div className="admin-dashboard__panel donor-dashboard__glass-panel h-100">
                ...
                <h2 className="landing-section__heading mb-3">{overviewHeading}</h2>
                <p className="landing-section__body mb-0">{overviewBody}</p>
                <p className="donor-overview-profile-eyebrow mt-4 mb-2">{profileEyebrow}</p>
                ...
                <div className="donor-trend mb-3" role="img" aria-label="Donation trend over the last six months">
                  <div className="donor-trend__chart">
                    ...
                    >
                      ...
                      {trendLineD ? (
                        <path className="donor-trend__stroke" d={trendLineD} fill="none" />
                      ) : null}
                      {trendCoords.map((p, i) => (
                        <circle
